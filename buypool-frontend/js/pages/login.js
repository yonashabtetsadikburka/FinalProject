...
export function destinazionePostLogin(utente) {
  if (!utente) return 'app.html#/';
  if (utente.ruolo === 'admin') return 'app.html#/admin';
  if (utente.ruolo === 'fornitore') return 'app.html#/area';
  return 'app.html#/';
}
...
window.handleCredentialResponse = async function(response) {
  ...
  try {
    const utente = await loginWithGoogle(response.credential);
    window.location.href = destinazionePostLogin(utente);
  } catch (err) {
...
async function handleMicrosoftLogin() {
  ...
  try {
    ...
    const utente = await loginWithMicrosoft(response.idToken);
    window.location.href = destinazionePostLogin(utente);
  } catch (err) {
...
export function LoginPage() {
  ...
  const attivato = query.get('attivato') === '1';
  if (attivato) {
    history.replaceState(null, '', window.location.pathname);
  }

  const attivatoBanner = attivato ? `
    <div role="alert" style="background: var(--color-success); color: #fff; border-radius: var(--radius-md); padding: var(--space-3); margin-bottom: var(--space-4); font-size: var(--text-sm);">
