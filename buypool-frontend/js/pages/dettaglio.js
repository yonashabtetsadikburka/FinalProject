...
let currentColletta = null;
let currentMiaPrenotazione = null;

function isPartecipato() {
  return currentMiaPrenotazione != null;
}

function getMiaPrenotazione() {
  return currentMiaPrenotazione;
}

async function handlePagaOra(prenotazioneId) {
  try {
    const res = await apiPost('/pagamento/checkout', { prenotazione_id: prenotazioneId });
    const url = res.dati?.url || res.url;
    if (url) {
...
function executePartecipa() {
  ...
  const colletta = currentColletta;
  if (!colletta) return;
...
async function loadDettaglio(id) {
  const { user } = getState();
  currentIdColletta = id;
  ...
  try {
    ...
    currentColletta = cRes.dati || null;

    const pren = (pRes.dati || []).find(p => p.id_colletta === parseInt(id) && p.id_utente === user?.id);
    currentMiaPrenotazione = pren || null;
  } catch (e) {
    currentColletta = null;
    currentMiaPrenotazione = null;
  }
}
...
function renderDettaglio() {
  ...
  const colletta = currentColletta;
  const percentage = Math.round((Number(colletta.quantita_attuale) || 0) / (Number(colletta.quantita_minima) || 0) * 100);
  const discount = Number(colletta.prezzo_base) > 0 ? Math.round((1 - Number(colletta.prezzo_corrente) / Number(colletta.prezzo_base)) * 100) : 0;
  const deadline = new Date(colletta.data_limite);
  const now = new Date();
  const daysLeft = Math.max(0, Math.ceil((deadline - now) / (1000 * 60 * 60 * 24)));
  const partecipato = isPartecipato();
  const miaPrenotazione = getMiaPrenotazione();
  const devePagare = miaPrenotazione && miaPrenotazione.stato === 'confermata';
  const haPagato = miaPrenotazione && (miaPrenotazione.stato === 'confermata' && miaPrenotazione.data_pagamento);
          ...
          ${Card({ children: `
            <div class="card-content">
              ...
              <div style="margin-bottom: var(--space-4);">
                ...
                ${Progress({ value: Number(colletta.quantita_attuale) || 0, max: Number(colletta.quantita_minima) || 0 })}
              </div>
          ...
          ${Card({ children: `
            <div class="card-content">
              ...
              <div style="display: flex; justify-content: space-between; margin-bottom: var(--space-3);">
                ...
                <span class="font-bold text-primary" style="font-size: var(--text-xl);">&euro;${Number(colletta.prezzo_corrente).toFixed(2)}</span>
              </div>
              <div style="display: flex; justify-content: space-between; margin-bottom: var(--space-3);">
                <span class="text-sm text-secondary">Prezzo base</span>
                <span class="text-sm" style="text-decoration: line-through;">&euro;${Number(colletta.prezzo_base).toFixed(2)}</span>
              </div>
              <div style="display: flex; justify-content: space-between; margin-bottom: var(--space-3);">
              ...
              ${devePagare ? `
                <button class="btn btn-success w-full" onclick="handlePagaOra(${miaPrenotazione.id})">
                  ...
                  Paga ora — &euro;${(Number(miaPrenotazione.importo_saldo) || 0).toFixed(2)}
                </button>
              ` : haPagato ? `
...
export async function DettaglioPage(params) {
  const content = document.getElementById('content-area') || document.querySelector('.main-content');
  if (!content) return;
  await loadDettaglio(params.id);
  renderDettaglio();
}

window.handlePartecipa = executePartecipa;
...
window.confirmPartecipa = async function() {
  closePartecipaModal();
  const { user } = getState();
  if (!user || !currentIdColletta) return;
  try {
    await apiPost(`/collette/${currentIdColletta}/partecipa`, { quantita: 1 });
  } catch (e) {
    const toast = document.createElement('div');
    toast.className = 'toast toast-error';
    toast.innerHTML = `<div class="toast-content"><div class="toast-title">Errore</div><div class="toast-description">${e.message || 'Impossibile partecipare'}</div></div>`;
    document.querySelector('.toast-container')?.appendChild(toast) || document.body.appendChild(toast);
    setTimeout(() => toast.remove(), 4000);
  }
  await loadDettaglio(currentIdColletta);
  renderDettaglio();
};
