...
window.showQr = function(qrJson) {
  const qr = typeof qrJson === 'string' ? JSON.parse(qrJson) : qrJson;
  if (!qr) return;
  renderQrModal(qr);
};
...
window.handleScanToken = async function() {
  ...
  resultDiv.innerHTML = `<div style="color: var(--color-text-secondary); font-size: var(--text-sm);">Verifica in corso...</div>`;
  try {
    const res = await apiPost('/prenotazioni/ritiro', { token });
    const dati = res.dati || {};
    resultDiv.innerHTML = `
      <div style="background: var(--color-success-light); border-radius: var(--radius-md); padding: var(--space-4);">
        ...
        <div style="font-size: var(--text-sm); margin-bottom: var(--space-2);">Prodotto: <strong>${dati.prodotto || 'Prodotto'}</strong></div>
        <div style="font-size: var(--text-sm); margin-bottom: var(--space-4);">Quantita: <strong>${dati.quantita ?? ''} pezzi</strong> &nbsp; Partecipante: <strong>${dati.partecipante?.nome || ''} ${dati.partecipante?.cognome || ''}</strong></div>
        <button class="btn btn-success" onclick="confirmScanRitiro()">
    ...
    resultDiv.innerHTML = `<div style="color: var(--color-error); font-size: var(--text-sm);">${e.message || 'Token non valido.'}</div>`;
  }
};

window.confirmScanRitiro = function() {
  const resultDiv = document.getElementById('scan-result');
  if (resultDiv) {
    resultDiv.innerHTML = `
...
export async function OrdiniPage() {
  ...
  try {
    const [pRes, sRes, tRes] = await Promise.all([
      ...
      apiGet('/sedi'),
      userIsAdmin ? apiGet('/prenotazioni') : Promise.resolve({ dati: [] })
    ]);
  ...
  const ordiniHtml = prenotazioni.length > 0 ? prenotazioni.map(p => {
    ...
    const qr = p.qr || null;
    const qrJson = qr ? encodeURIComponent(JSON.stringify({ token: qr.token, quantita_assegnata: qr.quantita_assegnata, stato: qr.stato })) : '';
        ...
        <div class="card-content">
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; margin-top: var(--space-3); padding-top: var(--space-3); border-top: 1px solid var(--color-border);">
            ...
            <div style="display: flex; align-items: center; gap: var(--space-2);">
              ...
              ${showQrButton ? `
                <button class="btn btn-default btn-sm" onclick="showQr('${qrJson}')">
  ...
  `).join('') || '<p class="text-secondary">Nessun punto di ritiro disponibile.</p>';

  // --- Sezione 3: Ordini pronti per ritiro ---
  const ordiniPronti = tutte.filter(p => {
    const qr = p.qr;
    return qr && qr.stato === 'generato';
  });

  const ordiniProntiHtml = ordiniPronti.length > 0 ? ordiniPronti.map(p => {
    const prodotto = p.colletta?.prodotto?.nome || 'Prodotto';
    const qr = p.qr;
    return `
      <div style="display: flex; justify-content: space-between; align-items: center; padding: var(--space-3) 0; border-bottom: 1px solid var(--color-border);">
        <div>
          <div class="font-medium">${prodotto}</div>
