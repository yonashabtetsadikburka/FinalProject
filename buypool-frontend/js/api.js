...
export async function apiRequest(method, url, body = null) {
  ...
  try {
    ...
    if (!response.ok) {
      // Le backend renvoie soit "errore" (string), soit "errori" (objet de
      // validation Laravel : { champ: ["message"] }) selon le type d'erreur.
      let messaggio = data.errore || 'Errore del server';
      if (data.errori) {
        const primoErrore = Object.values(data.errori)[0];
        messaggio = Array.isArray(primoErrore) ? primoErrore[0] : primoErrore;
      }
