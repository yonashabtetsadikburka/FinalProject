import { getFornitoreById, PRODOTTI, COLLETTE } from '../mock.js';
import { Card, CardHeader, CardTitle, CardContent } from '../components/card.js';

export function FornitoriDettaglioPage(params) {
  const content = document.getElementById('content-area') || document.querySelector('.main-content');
  if (!content) return;

  const fornitore = getFornitoreById(params.id);
  ...
  const productsHtml = fornitore.prodotti.map(p => `
    <div style="display: flex; justify-content: space-between; padding: var(--space-3) 0; border-bottom: 1px solid var(--color-border);">
      ...
      <div class="font-bold">&euro;${p.prezzo_unitario.toFixed(2)}</div>
    </div>
          ...
          ${Card({ children: `
            <div class="card-content">
              <div style="display: flex; align-items: center; gap: var(--space-4); margin-bottom: var(--space-4);">
                <div class="supplier-logo" style="width: 80px; height: 80px; font-size: var(--text-3xl);">${fornitore.nome_azienda[0]}</div>
                <div>
