...
import { getNotificheByUtente } from '../mock.js';
...
function renderMobileNavItems(hash) {
  ...
  if (isAdmin()) {
    ...
    html += '<div style="margin: var(--space-3) 0; border-top: 1px solid var(--color-border);"></div>';
  }
...
export function Header() {
  const { user } = getState();
  const notifiche = user ? getNotificheByUtente(user.id) : [];
    ...
    <header class="header">
      ...
      <div class="header-center">
        ...
        <nav class="header-nav">
          ${userIsAdmin ? renderAdminNav(hash) : renderNavItems(hash) + renderMioSpazioDropdown(hash)}
        </nav>
      </div>
